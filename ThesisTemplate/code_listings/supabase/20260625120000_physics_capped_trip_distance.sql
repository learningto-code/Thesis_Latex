-- that job correctly, but it ALSO truncates legitimate highway segments: a row
-- heartbeat/alerts on the single worker queue.
-- New per-segment cap:
-- where:
-- Why this works:
--     even more aggressively than the old 0.6 km hard cap.
--     counted in full.
--   * 1.3x slack covers cornering and route deviation between sparse points.
--     moving (radar-confirmed via GPS hav_km).
--   * 200 km/h ceiling and 0.05 km floor are sanity guards.
...
-- Recompute all ended trips so dashboard distance reflects the new physics cap.
UPDATE trip_sessions t
SET distance_km = public.compute_trip_distance_km(t.id)
WHERE trip_status = 'ended';

-- Re-accumulate per-truck odometer from scratch using the new distances.

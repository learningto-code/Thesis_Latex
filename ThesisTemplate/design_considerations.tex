...
\begin{enumerate}
    ...
    IEC\glsadd{IEC}~61131-2 and ISO~7637-2 were used as references for embedded-control and automotive transient considerations \cite{iec61131, iso7637}. Commercial off-the-shelf modules with available CE\glsadd{CE}/FCC\glsadd{FCC}-marked variants were selected where possible; however, full product-level certification was outside the scope of this prototype study \cite{cecert,fcc}.
...
\begin{itemize}  
    \item \textbf{Data Acquisition Layer:} Captures sensor and telemetry data, applies SMA and median filters, and performs anomaly threshold checks. Implementation details are in Sec.~\ref{sec:fuel_acquisition}.  

    \item \textbf{Processing Layer:} Performs edge anomaly detection, timestamping, and data aggregation. Standards for real-time processing and task scheduling ensure predictable latency. Implementation is in Sec.~\ref{sec:fuel_preprocessing}.  

...
\newcommand{\documentType}{Thesis Paper}
...
\newcommand{\department}{Department of Electronics, Computer, and Electrical Engineering}
...
\newcommand{\groupID}{EQ1-06} % group ID is for undergraduates as of this formatting
...
\defineAuthor{surname1}{Barja}
\defineAuthor{firstname1}{Samuelle P.}

\defineAuthor{surname2}{Castelo}
\defineAuthor{firstname2}{Konrad Romel C.}

\defineAuthor{surname3}{De La Rosa}
\defineAuthor{firstname3}{Eliza Rica R.}

\defineAuthor{surname4}{Montemayor}
\defineAuthor{firstname4}{Alyssa S.}


\newcommand{\documentTitle}{An IoT-Enabled System for Real-Time Fuel Monitoring, GPS Tracking, and Fuel Anomaly Detection in Trucks Using Context-Aware Non-Parametric Machine Learning} % put tilde (~) between words to indicate non-breaking adjacent words

\newcommand{\keywords}{Fuel Monitoring, GPS Tracking, Fuel Anomaly Detection, Internet of Things, Context-Aware Machine Learning, Isolation Forest, Matrix Profile}
...
% Chapter-style heading for glossary/abbreviation/notation printterms that does NOT force a new page.
% The default heading in datagidx is \chapter*, which triggers a clearpage and leaves the previous
% recto page blank. This variant preserves the visual chapter-title look but keeps content on the
% same page that was already cleared by the surrounding \cleardoublepage/\clearpage.
\makeatletter
\newcommand*{\gidxheading}[1]{%
	\begin{center}%
		{\Large\bfseries #1}%
	\end{center}%
	\par\vspace{2ex}%
...
\includeonly{% just comment those portions that you do not want to be included in the parsing
	...
	conclusions,
	appendix_partner_evidence,
	appendix_survey_instrument,
	appendix_technical_details,
	appendix_gantt,
	appendix_source_code,
	appendix_ai_disclosure,
...
\begin{document}
	...
	\begin{SingleSpace}
		...
		{
			\glsaddall{abbreviation}
			\printterms[database=abbreviation, style=indexalign, heading={\gidxheading}, prelocation=none, location=hide, columns=1, postname=\hspace{3em}]
			\thispagestyle{plain}
		}
		\clearpage
		...
		{
			\glsaddall{notation}
			\printterms[database=notation, style=indexalign, heading={\gidxheading}, prelocation=none, location=hide, columns=1, postname=\hspace{3em}]
			{
				\vspace{3ex}
		...
		{	
			\glsaddall{glossary}
			\printterms[database=glossary, style=indexalign, heading={\gidxheading}, prelocation=none, location=hide, columns=1, postname=\hspace{3em}]
			\thispagestyle{plain}
		}
		\cleardoublepage
	...
	%\bibliographystyle{apalike} % for APA referencing format (ignores DOI/URL)
	%\bibliographystyle{plainnat} % natbib author-year style (renders authors First-Last)
	\bibliographystyle{apacite} % APA style: surname-first author formatting so alphabetical order is visually obvious
	\nocite{*}
	\begin{SingleSpace}
	...
	\label{ch:appendix_urerb_decision_letter}
	\includepdf[pages=-,%
		offset=3.5mm -7mm,%
		scale=0.70,%
		pagecommand={},]
	{appendix_urerb_decision_letter.pdf}
	\cleardoublepage

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\chapter{Partner Coordination and Prototype Demonstration}
	\label{ch:appendix_partner_evidence}
	\include{appendix_partner_evidence}
	\cleardoublepage

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\chapter{Administrator Acceptance Survey Instrument}
	\label{ch:appendix_survey_instrument}
	\include{appendix_survey_instrument}
	\cleardoublepage

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\chapter{Technical Implementation Details}
	\label{ch:appendix_technical_details}
	\include{appendix_technical_details}
	\cleardoublepage

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\chapter{Individual Gantt Charts}
	\label{ch:appendix_gantt}
	\include{appendix_gantt}
	\cleardoublepage

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\chapter{Summary of Revisions Prior to Approval}
	\label{ch:rev_final}
	...
	\label{ch:appendix_source_code}
	\include{appendix_source_code}
	\cleardoublepage

	%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	\chapter{Generative AI Use Disclosure}
	\label{ch:appendix_ai_disclosure}
	\include{appendix_ai_disclosure}
	\cleardoublepage
